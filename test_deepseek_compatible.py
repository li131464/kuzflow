...
def test_deepseek_compatible():
    ...
    try:
        ...
        ocr_result_path = project_root / "temp_results" / "ocr_result.json"
        ocr_text_path = project_root / "temp_results" / "ocr_text.txt"
        
        if not ocr_result_path.exists():
            print(f"错误: 未找到OCR结果文件 - {ocr_result_path}")
            print("请先运行 OCR 环境: conda activate ocr_env && python test_ocr_compatible.py")
            return None
        ...
        print(f"\n=== 最终提取结果 ===")
        print(f"甲方公司名称: {extracted_info.get('company_name', '未找到')}")
        print(f"购买物品名称: {extracted_info.get('product_name', '未找到')}")
        print(f"购买物品数量: {extracted_info.get('product_quantity', '未找到')}")
        print(f"下订单日期: {extracted_info.get('order_date', '未找到')}")
